...
| [CLSO Pickle for Sight](pickle-for-sight-spec.md) | Instantiating the standard-tournament template for a one-day, nine-category, two-venue event on 27 Sep 2026, the first real standard-template run | Nothing. The QR image is the only input still to supply (§2) |
